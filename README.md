...
ECPU is aiming to take the next step to abstract away CPU and NET for users on the EOS mainnet, while also adding deflationary pressure to EOS by locking EOS into REX permanently. An initial EOS pool (staked to REX permanently) uses rewards to purchase CPU/NET for staked ECPU, the unused EOS reserved for unstaked ECPU is sent to REX to be locked permanently and to compound the permanent pool. ECPU is mined by providing EOS to the permanent REX pool. ECPU also allows you to vote for which proxy the permanent pool will delegate to.
